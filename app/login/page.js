   ...
   const [showpass, setshowpass] = useState(false)
    const onSubmit = async (data) => {
        ...
        if (res.error) {
            ...
            router.push(`/${data.Email.split('@')[0]}`);
        }
    };

    useEffect(() => {
        if (status === "loading") return; // avoid early redirect before auth is ready
        if (session) router.push(`/${session.user.email.split('@')[0]}`);//({`/${session.user.name}`});
    }, [session, status, router]);
    return (
        <div className='flex justify-center items-center bg-black-100'>
            <div className="flex flex-col gap-6 min-h-screen bg-white-100 p-10 w-90 mt-[20vh]">
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center items-center gap-4 text-white">
                    {/* register your input into the hook by invoking the "register" function */}
                    <input type="text" className="border-2 border-slate-600 rounded-2xl p-2 lg:w-[25vw] w-[60vw]" placeholder="Email" {...register("Email", {
                        required: { value: true, message: "Email required" }, validate: (value) =>
                            value.includes('@') && value.includes('.') || 'Please enter a valid email address', message: "invalid Email"
                    })} />
                    {errors.Email && <div className="text-white rounded-2xl text-xs">{errors.Email.message} </div>}
                    {/* include validation with required or other standard HTML validation rules */}
                   <div className='relative'>
                     <input type={showpass ? "text" : "password"} className="border-2 border-slate-600 rounded-2xl lg:w-[25vw] w-[60vw] p-2" placeholder='password' {...register("password", { required: { value: true, message: "password required" }, minLength: { value: 8, message: "minimum 8 characters required" } })} />
                    {errors.password && <div className="text-white rounded-2xl text-xs">{errors.password.message} </div>}
                    <img onClick={()=>{setshowpass(!showpass)}} className={`${showpass?"flex":"hidden"} h-5 right-3 invert-100 absolute top-3 w-5`} src='/hidepass.png'/>
                    <img onClick={()=>{setshowpass(!showpass)}} className={`${showpass?"hidden":"flex"} h-5 right-3 invert-100 absolute top-3 w-5`} src='/showpassw.png'/>
                   </div>
                    {/* errors will return when field validation fails  */}
                    <button className="flex items-center justify-center text-white bg-gradient-to-br cursor-pointer from-purple-600 to-violet-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-2xl text-sm px-5 py-2.5 text-center lg:w-[25vw] w-[60vw]">Log in</button>
